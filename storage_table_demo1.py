...
resourcegroup_name = 'ramesh'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'ramesh'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'westus'
...
car.cost = 46000
table_service.insert_entity('itemstable', car)
print('Created entry for Odyssey...\n')
...
print('Created entry for a coffee @starbucks...\n')
time.sleep(1)
...
coffee.RowKey = '005'
coffee.brand = 'philz'
coffee.flavor = 'ginger honey'
coffee.size = 'small'
coffee.cost = 3.50
table_service.insert_entity('itemstable', coffee)
print('Created entry for ginger honey @philz...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeestore'", select='brand,flavor')
for item in items:
